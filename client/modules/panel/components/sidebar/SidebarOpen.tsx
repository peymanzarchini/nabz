...
const SidebarOpen = ({
  ...
  return (
    ...
    >
      ...
      />
      <aside
        className={`relative w-72 bg-card h-full flex flex-col transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="h-20 flex items-center justify-between border-b border-border px-4">
          <div className="flex items-center">
            <Logo width={40} height={40} />
            <span className="text-lg font-black mr-2 text-foreground">داشبورد</span>
          </div>
          <Button
            variant="ghost"
            size="icon"
          ...
          {filteredLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors relative cursor-pointer ${
                  isActive
