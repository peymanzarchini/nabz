        ...
        {!isLoading && offers && offers.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {offers.map((offer, index: number) => (
              <Link
                key={offer.id}
                href={`/listings?categoryId=${offer.category.id}`}
                ...
                <div className="p-4 space-y-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span className="font-black text-base text-foreground">
                      {Number(offer.finalPrice || offer.price).toLocaleString("fa-IR")}
                      <span className="text-[10px] font-normal text-muted-foreground mr-1">
                        تومان
                      </span>
                    </span>

                    {Boolean(offer.discountPercentage && offer.price !== offer.finalPrice) && (
                      <span className="text-xs text-muted-foreground line-through">
                        {Number(offer.price).toLocaleString("fa-IR")}
                      </span>
                    )}
                  </div>
                </div>
